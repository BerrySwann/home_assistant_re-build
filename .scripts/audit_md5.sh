...
# ╰──────────────────────────────────────────────────────────────────────────╯
# Compare chaque fichier YAML entre la prod HA et GitHub (raw URL)
...
# Base URL raw GitHub (repo public — pas d'auth requise)
...
# ── PASS 3 : MD5 GITHUB (curl raw URL) ───────────────────────────────────
echo "── PASS 3 : MD5 GITHUB (curl raw) ──────────────────────────────────────────" >> "$LOG"
...
while IFS='|' read -r file prod_md5; do
    github_raw=$(curl -sf --max-time 15 "${GH_BASE}/${file}" 2>/dev/null || true)
...
# [2026-06-14] FIX : git fetch origin (sans branche) au lieu de git fetch origin main
# [2026-06-15] FIX RADICAL : abandon git show origin/main:file (ref stale non résolvable)
#              → remplacement par curl -sf --max-time 15 "${GH_BASE}/${file}"
#              → curl pipe md5sum : zéro /tmp write, RAM nulle, fiable et direct
