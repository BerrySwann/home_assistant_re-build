#!/usr/bin/env bash
# ╭──────────────────────────────────────────────────────────────────────────╮
# │ AUDIT MD5 — 4 PASSES : SNAPSHOT → TREE → MD5 PROD → MD5 GITHUB → RAPPORT │
# ╰──────────────────────────────────────────────────────────────────────────╯
...
DIRS=$(find . -maxdepth 1 -mindepth 1 -type d ! -name ".*" ! -name "__pycache__" | sed "s|^\./||" | sort | tr "\n" " ")
...
# ── PASS 0 : TREE SNAPSHOT (yaml + sh + txt) ──────────────────────────────
TREE_SNAPSHOT="$LOG_DIR/tree_$(date '+%Y-%m-%d').txt"
{
echo "╭──────────────────────────────────────────────────────────────────────────────╮"
echo "│ TREE SNAPSHOT — $(date '+%Y-%m-%d %H:%M:%S %Z')"
echo "╰──────────────────────────────────────────────────────────────────────────────╯"
echo ""
echo "── .yaml ─────────────────────────────────────────────────────────────────────"
find . \( -path "./.git/*" -o -path "./.storage/*" -o -path "./.*" \) -prune \
     -o -name "*.yaml" -print | sed 's|^\./||' | sort
echo ""
echo "── .sh ───────────────────────────────────────────────────────────────────────"
find . \( -path "./.git/*" -o -path "./.*" \) -prune \
     -o -name "*.sh" -print | sed 's|^\./||' | sort
echo ""
echo "── .txt ──────────────────────────────────────────────────────────────────────"
find . \( -path "./.git/*" -o -path "./.*" \) -prune \
     -o -name "*.txt" -print | sed 's|^\./||' | sort
} > "$TREE_SNAPSHOT"
echo "  → Snapshot : $TREE_SNAPSHOT"
# ── PASS 1 : TREE ─────────────────────────────────────────────────────────
{
echo "╭──────────────────────────────────────────────────────────────────────────────╮"
...
# [2026-06-28] PASS 0 ajouté : snapshot yaml+sh+txt → tree_YYYY-MM-DD.txt dans .logs/
#              DIRS dynamique (auto-découverte) — packages retiré (dossier supprimé)
