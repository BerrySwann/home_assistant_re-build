...
[![Modifié](https://img.shields.io/badge/MàJ-2026--06--16-44739e?style=flat-square)](.)&nbsp;
...
| Champ | Valeur |
|:------|:-------|
| 📁 **Path** | `Dashboard/L5C3_15_MariaDB/page_L5C3_mariadb_2026-06-15.yaml` |
...
- **Audit MD5** : bouton déclencheur `script.audit_md5`, résultat lisible via `sensor.audit_md5_journal`.
...
├─────────────────────────────────────────────┤
│  SEPARATOR : Audit GitHub MD5               │
├─────────────────────────────────────────────┤
│  vertical-stack                             │
│  ├── button-card : script.audit_md5         │
│  │    (bleu idle / orange spinning en cours)│
│  └── markdown : résultat + chemin log       │
│       (sensor.audit_md5_journal attr:text)  │
├─────────────────────────────────────────────┤
│  SEPARATOR : GitHub                         │
├─────────────────────────────────────────────┤
│  vertical-stack                             │
...
`custom:button-card` sur `script.audit_md5`. Deux états visuels :

| État script | Fond | Icône | Couleur |
|:------------|:-----|:------|:--------|
| `off` (idle) | bleu `rgba(3,169,244,0.08)` | `mdi:shield-search` | `#03a9f4` |
| `on` (en cours) | orange `rgba(255,152,0,0.10)` | `mdi:rotate-right` rotating | `#ff9800` |

Bouton droit (custom_field `btn`) : même logique icône + animation `rotating 1s linear infinite`.
...
| Entité | unique_id | Rôle |
|--------|-----------|------|
| `sensor.audit_md5_journal` | `audit_md5_journal` | Lit les 25 dernières lignes de `/config/.logs/md5_audit_latest.txt` → attr `text` |
...
1. Vérifier que `shell_command.git_backup_push_manual` et `shell_command.git_backup_push_weekly` existent (`shell_command.yaml`)
...
| Élément | Type | Statut |
|---------|------|--------|
| `script.audit_md5` | `scripts.yaml` | ✅ Essentiel |
| `sensor.audit_md5_journal` | `command_line/audit/audit_logs.yaml` | ✅ Essentiel |
...
### Configuration YAML
- `scripts.yaml` (`script.audit_md5`)
- `command_line/audit/audit_logs.yaml` (`sensor.audit_md5_journal`)
- `command_line/github_maintenance/github_maintenance.yaml`
- `shell_command.yaml`
- `sql.yaml`
