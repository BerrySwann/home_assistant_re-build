...
*Validée le 2026-04-29 — Migration TPL kWh complète (_um → _um_kwh_tpl) | Dashboard archivé le 2026-05-13*
...
| `light.lampe_salle_de_bain_hue` | NAT | Hue Bridge | Badge état SDB |
...
| `light.lampe_salle_de_bain_hue` | NAT | Hue Bridge |
...
| `light.lampe_salle_de_bain_hue` | NAT | Hue Bridge | Mushroom toggle SdB + heading badge |
...
```
MATÉRIEL / INTÉGRATION
  ├─→ Intel NUC host Proxmox → publish_temp.sh (systemd 10s) → MQTT Discovery
  │     └─→ sensor.proxmox_cpu_package  (NAT/MQTT)
  │           └─→ TPL: templates/Mini-PC/MP_02_sonde_température_mini-pc.yaml
  │                 └─→ sensor.temperature_cpu_package  ← entité principale (icône + champ temp)
  ├─→ system_monitor (intégration HA)
  │     ├─→ sensor.system_monitor_utilisation_du_processeur  (NAT)  ← CPU %
  │     ├─→ sensor.system_monitor_utilisation_de_la_memoire  (NAT)  ← RAM %
  │     └─→ sensor.system_monitor_utilisation_du_disque      (NAT)  ← Disk %
  └─→ Zigbee2MQTT (prise IKEA Inspelning)
        └─→ sensor.prise_mini_pc_ikea_power  (NAT)   ← Watts instantanés
              └─→ VIGNETTE L4C2 (button-card 7 zones CSS — icône phu:intel-nuc)
```
...
| Entité | Type | Fichier source | Rôle |
|:-------|:----:|:--------------|:-----|
| `sensor.temperature_cpu_package` | TPL | `templates/Mini-PC/MP_02_sonde_température_mini-pc.yaml ← MQTT` | Entité principale — icône + champ `temp` |
...
MATÉRIEL / INTÉGRATION
  ...
  ├─→ Intel NUC host Proxmox → publish_temp.sh (systemd 10s) → MQTT Discovery
  │     └─→ sensor.proxmox_cpu_package / carte_mere / core_0/1/2/3  (NAT/MQTT)
  │           └─→ TPL: templates/Mini-PC/MP_02_sonde_température_mini-pc.yaml
  │                 ├─→ sensor.temperature_cpu_package   ← Blocs 2, 9, 11
  │                 ├─→ sensor.temperature_core_0 / 1 / 2 / 3  ← Bloc 10
...
| Fichier | Statut |
|:--------|:------:|
| `Dashboard/L4C2_11_Mini_PC/vignette_L4C2_mini_pc_2026-05-10.yaml` | ✅ |
| `Dashboard/L4C2_11_Mini_PC/page_L4C2_mini_pc_2026-05-10.yaml` | ✅ |
| `Dashboard/L4C2_11_Mini_PC/page_L4C2_mini_pc_2026-05-14.yaml` | ✅ |
| `templates/Mini-PC/MP_02_sonde_température_mini-pc.yaml` | ✅ |
...
Atmo France (intégration HACS) — Section POLLENS
  ├─→ sensor.qualite_globale_pollen_vence    (NAT) → entity-progress-card-template (barre globale)
...
Atmo France (intégration HACS) — Section QUALITE DE L'AIR
  ├─→ sensor.qualite_globale_vence           (NAT) → entity-progress-card-template (barre globale)
  │     └─→ state_attr('Libellé') + state_attr('Couleur') → label + bar_color dynamiques
  ...
  └─→ sensor.pm25_vence                      (NAT) → ring-tile PM25
        ...
        └─→ PAGE L6C2 — 2 sections (POLLENS / QUALITE DE L'AIR)
              ├─→ custom:entity-progress-card-template × 2 (barres globales)
