...
| `climate.clim_chambre_nodon` | `set_temperature` (target_temp_low/high — NodOn Z2M) |
...
- Les températures eco ont un fallback : si `_corrige` indisponible → `_hiver`/`_ete` brut.
