...
[![MàJ](https://img.shields.io/badge/MàJ-2026--06--19-44739e?style=flat-square)](.)&nbsp;
...
## 🔧 SCRIPTS BASH — MAINTENANCE SYSTÈME

> Emplacement prod : `/homeassistant/.scripts/`
> Emplacement local : `docs_scripts/TREE_CORRIGE/.scripts/`

| Fichier | Rôle | Sortie |
|:--------|:-----|:-------|
| `audit_md5.sh` | Audit MD5 GitHub ↔ prod (sensors/templates/utility_meter/command_line) | `/homeassistant/.logs/audit_yaml.log` |
| `ha_git_backup.sh` | Backup git automatique HA → GitHub | GitHub `home_assistant_re-build` |
| `prod_tree.sh` | Génère l'arborescence complète yaml/sh/txt de la prod | `/homeassistant/prod_tree_YYYY-MM-DD.txt` |

**Chemin réel HA** : `/homeassistant/` (et non `/config/` — symlink non suivi par `find`)
