...
script.j_1_x_<piece>_clim_on_off_intelligent  (sur OFF)
    ...
    └── script.j_2_0_secu_arret_clim_protege  (mode: parallel, max: 3)
            ...
            ├── 2. Thermostat OFF  climate.clim_<piece>_rm4_mini (salon/bureau) ou clim_chambre_rm4_mini → hvac_mode: off
...
j_2_0_secu_arret_clim_protege:
  ...
  variables:
    ...
    clim_entity: "{% if p == 'chambre' %}climate.clim_chambre_rm4_mini{% else %}climate.clim_{{ p }}_rm4_mini{% endif %}"
...
| `climate.clim_<piece>_rm4_mini` (salon/bureau) / `climate.clim_chambre_rm4_mini` (chambre) | Climate | Mise en mode off du thermostat |
